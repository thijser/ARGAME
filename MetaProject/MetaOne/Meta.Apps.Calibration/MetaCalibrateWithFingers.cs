...
namespace Meta.Apps.Calibration
{
	internal class MetaCalibrateWithFingers : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			this.instructions.get_transform().get_parent().GetComponent<MetaBody>().hud = true;
		...
		private void Update()
		{
			if (Hands.right != null)
			{
				...
				{
					this.handOutline.get_transform().set_localPosition(new Vector3(0f, 0f, 0.399f) + new Vector3(0f, 0f, -0.05f) * (1f + Mathf.Sin(4f * Time.get_time())) / 2f);
					...
					{
						float num = this.fingerTracker.get_transform().get_localPosition().x * this.scalingFactor;
						float num2 = this.fingerTracker.get_transform().get_localPosition().y * this.scalingFactor;
						...
						instance.CalibrationValue = (this.fingerTracker.get_transform().get_localPosition().z - 0.3f) * 2f;
					...
					{
						Debug.Log("Sensor Screen Offset in camera profile should be (-0.01275, -0.025, 0)!");
					}
					if (MetaCamera.GetCameraProfile().m_sensorScreenAngle != -5f)
					{
						Debug.Log("Problem: Sensor Screen Angle in camera profile should be -5!");
					}
					if (this.stable)
					{
						float num4 = (Time.get_time() - this.stableStartTime) / this.confirmationTime;
						if (this.playStableSound)
						{
							base.GetComponent<AudioSource>().set_clip(this.countDownBegin);
							base.GetComponent<AudioSource>().Play();
							this.playStableSound = false;
						}
						this.fingerMessage.set_text(string.Format("Hold still: {0}", Mathf.Ceil((1f - num4) * 5f)));
						this.stallIndicator.get_transform().set_localScale(0.04f * new Vector3(1f - num4, 1f - num4, 1f - num4));
						if (num4 > 1f)
						{
							base.GetComponent<AudioSource>().set_clip(this.countDownFinish);
							base.GetComponent<AudioSource>().Play();
					...
					else
					{
						...
						this.stallIndicator.get_transform().set_localScale(new Vector3(0f, 0f, 0f));
		...
		public void SetReach(float reachDist)
		{
			...
			base.GetComponent<AudioSource>().set_clip(this.calibrationComplete);
			base.GetComponent<AudioSource>().Play();
		...
		private void setVisibility()
		{
			...
			this.interactionZoneVisualizer.get_transform().set_localScale(new Vector3(this.maxCorrectionDistance / this.scalingFactor, (num2 - num) / 2f, this.maxCorrectionDistance / this.scalingFactor));
			this.interactionZoneVisualizer.get_transform().set_localPosition(new Vector3(0f, 0f, (num2 + num) / 2f));
			...
			else
			{
				...
				else if (this.design == MetaCalibrateWithFingers.CalibrationDesign.Fingertip)
				{
					...
					{
						this.basketball.get_transform().Rotate(0f, Time.get_deltaTime() * 250f, 0f);
					}
				}
