...
namespace Meta
{
	public class MarkerDetector : MetaSingleton<MarkerDetector>, IMetaEventReceiver
	{
		...
		public void GetMarkerTransform(int markerID, ref Transform newTransform)
		{
			...
			{
				Matrix4x4 m = base.get_transform().get_localToWorldMatrix() * this.markerTransformDict[markerID];
				newTransform.set_position(m.PositionFromMatrix());
				newTransform.set_rotation(m.QuaternionFromMatrix());
				newTransform.set_position(newTransform.get_position() + this.markerOffset);
