	...
	public class MarkerTargetIndicator : MonoBehaviour
	{
		...
		public static void HighlightMarker(int markerID, float distance)
		{
			if (MarkerTargetIndicator.markerIndicators.ContainsKey(markerID))
			{
				...
				highlightableObject.set_enabled(true);
				float num = 0.05f;
		...
		private void UpdateMarkerIndicators()
		{
			foreach (int current in MetaSingleton<MarkerDetector>.Instance.updatedMarkerTransforms)
			{
				...
				{
					GameObject gameObject = (GameObject)Object.Instantiate(this._markerIndicatorBase);
					if (MarkerTargetIndicator._indicatorParent == null)
					{
						MarkerTargetIndicator._indicatorParent = new GameObject();
						MarkerTargetIndicator._indicatorParent.set_name("MarkerIndicators");
					...
					gameObject.set_name("MarkerIndicator" + current);
					...
					{
						gameObject.GetComponentsInChildren<Renderer>(true)[0].get_material().set_mainTexture(mainTexture);
					}
		...
		private void Update()
		{
			if (this._isController && MetaSingleton<MarkerDetector>.Instance != null && MetaSingleton<MarkerDetector>.Instance.get_enabled())
			{
