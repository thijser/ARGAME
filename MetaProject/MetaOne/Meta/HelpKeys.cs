...
namespace Meta
{
	internal class HelpKeys : MonoBehaviour
	{
		...
		private void UpdateKeyText(GameObject key, string newKeyText)
		{
			...
			{
				key.GetComponent<Text>().set_text(newKeyText.ToUpper());
			}
		}
		...
		private void Update()
		{
			if (Input.GetKeyDown(MetaSingleton<KeyboardShortcuts>.Instance.toggleHelpPanel))
			{
				...
				{
					this.HelpPanel.GetComponent<RectTransform>().set_localScale(new Vector3(0.00025f, 0.00025f, 0.00025f));
				}
				else
				{
					this.HelpPanel.GetComponent<RectTransform>().set_localScale(new Vector3(0.00017f, 0.00017f, 0.00017f));
				}
