	...
	public class InputIndicators : MetaSingleton<InputIndicators>
	{
		...
		private void InitFingertipIndicatorTransforms()
		{
			for (int i = 0; i < 2; i++)
			{
				...
				{
					this._fingerPointerIndicatorTransforms[i] = this.InstantiatePrefab(this._fingertipIndicator, "FingertipIndicator", Hands.GetHands()[i].pointer.gameObject.get_transform(), new Vector3(0.001f, 0.001f, 0.001f), null);
					this._fingerPointerHoverTransforms[i] = this.InstantiatePrefab(this._hoverIndicator, "HoverIndicator", Hands.GetHands()[i].pointer.gameObject.get_transform(), new Vector3(0.001f, 0.001f, 0.001f), null);
					this._fingerPointerHoverTransforms[i] = this.InstantiatePrefab(this._dwellIndicator, "PointerDwellIndicator", Hands.GetHands()[i].pointer.gameObject.get_transform(), new Vector3(0.002f, 0.002f, 0.002f), null);
					for (int j = 0; j < 5; j++)
					{
						this._fingertipIndicatorTransforms[j + i * 5] = this.InstantiatePrefab(this._fingertipIndicator, "FingertipIndicator", Hands.GetHands()[i].fingers[j].gameObject.get_transform(), new Vector3(0.001f, 0.001f, 0.001f), null);
						this._fingertipHoverTransforms[j + i * 5] = this.InstantiatePrefab(this._hoverIndicator, "HoverIndicator", Hands.GetHands()[i].fingers[j].gameObject.get_transform(), new Vector3(0.001f, 0.001f, 0.001f), null);
					}
				}
			}
		}
		...
		private void InitGrabCircleTransforms()
		{
			for (int i = 0; i < 2; i++)
			{
				if (Hands.GetHands()[i] != null)
				{
					this._grabCircleSolidTransforms[i] = this.InstantiatePrefab(this._grabCircleSolid, "GrabCircleSolid", Hands.GetHands()[i].palm.gameObject.get_transform(), new Vector3(0.003f, 0.003f, 0.003f), new Color?(new Color(1f, 0.647058845f, 0f, 1f)));
					this._grabCircleDottedTransforms[i] = this.InstantiatePrefab(this._grabCircleDotted, "GrabCircleDotted", Hands.GetHands()[i].palm.gameObject.get_transform(), new Vector3(0.003f, 0.003f, 0.003f), new Color?(new Color(1f, 1f, 0.392156869f, 1f)));
				}
			}
		}
		...
		{
			Transform transform = Hands.GetHands()[(int)hand].pointer.gameObject.get_transform().FindChild("PointerDwellIndicator");
			transform.GetComponent<Renderer>().set_enabled(true);
			...
			transform.GetComponent<Renderer>().get_material().set_mainTexture(this._dwellIndicatorTextures[num]);
		...
		internal void HideDwellIndicators(HandType hand)
		{
			...
			{
				Transform transform = Hands.GetHands()[(int)hand].pointer.gameObject.get_transform().FindChild("PointerDwellIndicator");
				transform.GetComponent<Renderer>().set_enabled(false);
			}
		}

		private Transform InstantiatePrefab(GameObject prefab, string name, Transform parent, Vector3 localScale, Color? color = null)
		{
			Transform transform = ((GameObject)Object.Instantiate(prefab)).get_transform();
			transform.set_name(name);
			transform.set_parent(parent);
			transform.set_localPosition(Vector3.get_zero());
			transform.set_localScale(localScale);
			if (color.HasValue)
			{
				transform.GetComponent<Renderer>().get_material().set_color(color.Value);
				transform.GetComponent<Renderer>().get_material().SetColor("_SpecColor", color.Value);
			}
			return transform;
		}
		...
		private void SetFingertipRenderers(bool pointers, bool fingertips)
		{
			for (int i = 0; i < 2; i++)
			{
				...
				{
					this._fingerPointerIndicatorTransforms[i].GetComponent<Renderer>().set_enabled(pointers);
					if (!pointers)
					{
						this._fingerPointerHoverTransforms[i].GetComponent<Renderer>().set_enabled(false);
					}
				}
			}
			for (int j = 0; j < 10; j++)
			{
				...
				{
					this._fingertipIndicatorTransforms[j].GetComponent<Renderer>().set_enabled(fingertips);
				}
				if (!fingertips && this._fingertipHoverTransforms[j] != null)
				{
					this._fingertipHoverTransforms[j].GetComponent<Renderer>().set_enabled(false);
				}
			}
		}
		...
		{
			hoverIndicator.GetComponent<Renderer>().set_enabled(false);
		}
		...
		internal void UpdateHoverIndicator(Transform hoverIndicator, float distance)
		{
			if (this._fingertipIndicators && this._hoverIndicators)
			{
				...
				num /= 100f;
				hoverIndicator.set_localScale(new Vector3(num, num, num));
				hoverIndicator.GetComponent<Renderer>().set_enabled(true);
			}
		}

		internal void ColorHoverIndicator(Transform hoverIndicator, Color color)
		{
			hoverIndicator.GetComponent<Renderer>().get_material().set_color(color);
		}
		...
		public void InstantiatePressSound(Vector3 pos, AudioClip pressSound)
		{
			...
			{
				GameObject gameObject = Object.Instantiate(this._pressSound, pos, Quaternion.get_identity()) as GameObject;
				if (pressSound != null)
				{
					gameObject.GetComponent<AudioSource>().set_clip(pressSound);
				}
				gameObject.GetComponent<AudioSource>().set_loop(false);
		...
		{
			InputIndicators.<DestroySound>c__Iterator4 <DestroySound>c__Iterator = new InputIndicators.<DestroySound>c__Iterator4();
			<DestroySound>c__Iterator.sound = sound;
			<DestroySound>c__Iterator.<$>sound = sound;
			return <DestroySound>c__Iterator;
		}
		...
		internal void UpdateGrabCircles(int hand, bool objectWithinRange)
		{
			if (this._grabCircles && (objectWithinRange || this._grabCirclesAlways))
			{
				if (Hands.GetHands()[hand] != null)
				{
					...
					this._grabCircleSolidTransforms[hand].LookAt(Camera.get_main().get_transform());
		...
		private void SetCircleGrabbedColor(int hand, bool objectWithinRange)
		{
			...
			{
				this._grabCircleSolidTransforms[hand].GetComponent<Renderer>().get_material().set_color(Color.get_green());
				this._grabCircleSolidTransforms[hand].GetComponent<Renderer>().get_material().SetColor("_Emission", Color.get_green());
			}
			else
			{
				this._grabCircleSolidTransforms[hand].GetComponent<Renderer>().get_material().set_color(Color.get_gray());
				this._grabCircleSolidTransforms[hand].GetComponent<Renderer>().get_material().SetColor("_Emission", Color.get_gray());
			}
		}
		...
		private void SetCircleScale(int hand, float scale)
		{
			...
			vector = Vector3.Lerp(this._grabCircleDottedTransforms[hand].get_localScale(), vector, 10f * Time.get_deltaTime());
			this._grabCircleDottedTransforms[hand].set_localScale(vector);
		}
		...
		private void ToggleGrabCircle(int hand, bool visibility)
		{
			...
			{
				this._grabCircleSolidTransforms[hand].GetComponent<Renderer>().set_enabled(visibility);
			}
			if (this._grabCircleDottedTransforms[hand] != null)
			{
				this._grabCircleDottedTransforms[hand].GetComponent<Renderer>().set_enabled(visibility);
			}
		}
	}
}
