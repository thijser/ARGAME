...
namespace Meta
{
	internal class GestureManager : MetaSingleton<GestureManager>
	{
				...
				for (int i = 0; i < 2; i++)
				{
					if (this._handGestureData[i].m_hand != null)
					{
						if (this._handGestureData[i].m_currMode != MetaGesture.NONE)
						{
							...
							{
								this._handGestureData[i].m_controlledObj.SendMessage("OnHold", this._handGestureData[i].m_handType, 1);
								if (component.moveObjectOnGrab)
								{
									this._handGestureData[i].m_controlledObj.SendMessage("OnGrabMove", this._handGestureData[i].m_handType, 1);
								...
								{
									this._handGestureData[i].m_controlledObj.SendMessage("OnGrabRotate", this._handGestureData[i].m_handType, 1);
									Vector3 handsVector = this._handGestureData[i].m_hand.palm.position - this._handGestureData[i].m_controlledObj.get_position();
								...
								{
									this._handGestureData[i].m_controlledObj.SendMessage("OnGrabScale", this._handGestureData[i].m_handType, 1);
									float handDist = Vector3.Distance(this._handGestureData[i].m_hand.palm.position, Camera.get_main().get_transform().get_position());
							...
							{
								this._handGestureData[i].m_controlledObj.SendMessage("OnPinchHold", this._handGestureData[i].m_handType, 1);
								if (component.moveObjectOnPinch)
								{
									this._handGestureData[i].m_controlledObj.SendMessage("OnPinchMove", this._handGestureData[i].m_handType, 1);
									this._physics.MoveObj(this._handGestureData[i].m_controlledObj, this._handGestureData[i].m_hand.palm.position, this._handGestureData[i].m_initialObjOffset);
								}
								if (component.scaleObjectOnPinch)
								{
									this._handGestureData[i].m_controlledObj.SendMessage("OnPinchScale", this._handGestureData[i].m_handType, 1);
									float handDist2 = Vector3.Distance(this._handGestureData[i].m_hand.palm.position, Camera.get_main().get_transform().get_position());
						...
						else
						{
							...
							this._handGestureData[i].m_prevPalmPos = Camera.get_main().get_transform().get_position();
						}
		...
		private void StartTwoHandedGesture(Transform closestObj)
		{
			...
			this._initialObjOffset = (Hands.right.palm.position + Hands.left.palm.position) / 2f - this._controlledObj.get_position();
		...
		{
			float num = Vector3.Distance(handGestureData.m_prevPalmPos, Camera.get_main().get_transform().get_position());
			float num2 = Vector3.Distance(handGestureData.m_hand.palm.position, Camera.get_main().get_transform().get_position());
			if (num2 < this._minBound && num2 < num && num != 0f)
			{
				...
				if (Vector3.Distance(vector, Camera.get_main().get_transform().get_position()) < this._minBound)
				{
					vector = handGestureData.m_prevPalmPos + 0.1f * forward;
				}
				LeanTween.move(handGestureData.m_controlledObj.get_gameObject(), vector, 1f).setEase(18);
		...
		private void CheckTouchGesture(GestureManager.HandGestureData handGestureData)
		{
			...
			Collider[] array = Physics.OverlapSphere(handGestureData.m_hand.pointer.gameObject.get_transform().get_position(), ((SphereCollider)handGestureData.m_hand.pointer.gameObject.GetComponent<Collider>()).get_radius());
			Collider[] array2 = array;
			for (int i = 0; i < array2.Length; i++)
			{
				Collider collider = array2[i];
				if (MetaSingleton<MetaManager>.Instance.touchables.Contains(collider.get_transform()) && (collider.GetComponent<MetaBody>().useDefaultTouchSettings || collider.GetComponent<MetaBody>().touchingHandType == HandType.EITHER || collider.GetComponent<MetaBody>().touchingHandType == handGestureData.m_handType))
				{
					list.Add(collider.get_gameObject());
					if (handGestureData.m_touchedObjects.Contains(collider.get_gameObject()))
					{
						collider.get_gameObject().SendMessage("OnTouchHold", handGestureData.m_handType, 1);
						...
						GameObject expr_F2 = gameObject = collider.get_gameObject();
						float num = touchDwellTimers[gameObject];
						expr_E9[expr_F2] = num + Time.get_deltaTime();
						...
						float completion = handGestureData.m_touchDwellTimers[collider.get_gameObject()] / touchDwellTime;
						if ((collider.GetComponent<MetaBody>().touchableDwellable || collider.GetComponent<MetaBody>().useDefaultTouchSettings) && (handGestureData.stable || (!collider.GetComponent<MetaBody>().useDefaultTouchSettings && !collider.GetComponent<MetaBody>().touchDwellMustBeStable)))
						{
							...
							GameObject expr_1A7 = gameObject = collider.get_gameObject();
							num = touchDwellTimers2[gameObject];
							expr_19E[expr_1A7] = num + Time.get_deltaTime();
						...
						{
							MetaSingleton<InputIndicators>.Instance.HideDwellIndicators(handGestureData.m_handType);
							handGestureData.m_touchDwellTimers[collider.get_gameObject()] = 0f;
						}
						if (collider.GetComponent<MetaBody>().touchableDwellable && handGestureData.m_touchDwellTimers[collider.get_gameObject()] >= this._touchDwellTime)
						{
							collider.get_gameObject().SendMessage("OnTouchDwell", handGestureData.m_handType, 1);
						}
					}
					else
					{
						handGestureData.m_touchedObjects.Add(collider.get_gameObject());
						collider.get_gameObject().SendMessage("OnTouchEnter", handGestureData.m_handType, 1);
						handGestureData.m_touchDwellTimers.Add(collider.get_gameObject(), 0f);
					}
				}
			}
			foreach (GameObject current in handGestureData.m_touchedObjects)
			{
				...
				{
					current.SendMessage("OnTouchExit", handGestureData.m_handType, 1);
					if (handGestureData.m_touchDwellTimers[current] >= this._touchDwellTime && current.GetComponent<MetaBody>().touchableDwellable)
					{
						current.SendMessage("OnTouchDwellExit", handGestureData.m_handType, 1);
					}
					MetaSingleton<InputIndicators>.Instance.HideDwellIndicators(handGestureData.m_handType);
		...
		private void CheckPointGesture(GestureManager.HandGestureData handGestureData)
		{
			...
			if (handGestureData.m_hand.isValid && objectOfInterest != null && MetaSingleton<MetaManager>.Instance.pointables.Contains(objectOfInterest.get_transform()) && (objectOfInterest.GetComponent<MetaBody>().useDefaultPointSettings || objectOfInterest.GetComponent<MetaBody>().pointingHandType == HandType.EITHER || objectOfInterest.GetComponent<MetaBody>().pointingHandType == handGestureData.m_handType))
			{
				if (handGestureData.m_pointedAtObject == objectOfInterest)
				{
					objectOfInterest.SendMessage("OnPointHold", handGestureData.m_handType, 1);
					...
					if ((objectOfInterest.GetComponent<MetaBody>().useDefaultPointSettings || objectOfInterest.GetComponent<MetaBody>().pointableDwellable) && (handGestureData.stable || (!objectOfInterest.GetComponent<MetaBody>().useDefaultPointSettings && !objectOfInterest.GetComponent<MetaBody>().pointDwellMustBeStable)))
					{
						...
						{
							objectOfInterest.get_gameObject().SendMessage("OnPointDwell", handGestureData.m_handType, 1);
						}
				...
				{
					objectOfInterest.SendMessage("OnPointEnter", handGestureData.m_handType, 1);
					handGestureData.m_pointedAtTime = 0f;
					if (handGestureData.m_pointedAtObject != null)
					{
						handGestureData.m_pointedAtObject.SendMessage("OnPointExit", handGestureData.m_handType, 1);
					}
		...
		private Transform FindClosestObj(GestureManager.HandGestureData handGestureData, MetaGesture gesture)
		{
			...
			foreach (Transform current in list)
			{
				if (current != null)
				{
					...
					if (component != null && (handGestureData == null || component.gesture == MetaGesture.NONE) && current.get_gameObject().get_activeInHierarchy() && (handGestureData != null || component.twoHandedGrabbable))
					{
						float num2 = Vector3.Distance(vector, current.get_position());
						if ((num2 < this.GesturableDistance(component, gesture) || (current.GetComponent<Collider>() != null && current.GetComponent<Collider>().get_bounds().Contains(vector))) && (this.GestureHandType(component, gesture) == HandType.EITHER || handGestureData == null || this.GestureHandType(component, gesture) == handGestureData.m_handType) && (num == 0f || num2 < num))
						{
							result = current;
							num = num2;
						}
		...
		private void FindNextGrabbaleObj(GestureManager.HandGestureData handGestureData)
		{
			...
			if (transform != null && handGestureData.m_closestObj != null)
			{
				...
				float num = Vector3.Distance(position, transform.get_position());
				float num2 = Vector3.Distance(position, handGestureData.m_closestObj.get_position());
		...
		private void SetControlledObj(GestureManager.HandGestureData handGestureData, Transform controlledObj)
		{
			...
			if ((handGestureData.m_currGesture == MetaGesture.GRAB && this._enableGrab) || (handGestureData.m_currGesture == MetaGesture.PINCH && this._enablePinch))
			{
				...
				handGestureData.m_initialObjOffset = handGestureData.m_hand.palm.position - controlledObj.get_position();
				handGestureData.m_prevObjRot = controlledObj.get_rotation();
				handGestureData.m_prevHandVector = handGestureData.m_hand.palm.position - handGestureData.m_controlledObj.get_position();
				handGestureData.m_initialDistance = Vector3.Distance(handGestureData.m_hand.palm.position, Camera.get_main().get_transform().get_position());
				...
				if (handGestureData.m_currGesture == MetaGesture.GRAB && this._enableGrab)
				{
					...
					component.gesture = MetaGesture.GRAB;
					component.grabbed = true;
					controlledObj.SendMessage("OnGrab", handGestureData.m_handType, 1);
					this._gestureSound.set_clip(this._grabOn);
				}
				else if (handGestureData.m_currGesture == MetaGesture.PINCH && this._enablePinch)
				{
					...
					controlledObj.SendMessage("OnPinch", handGestureData.m_handType, 1);
		...
		private void ReleaseObj(GestureManager.HandGestureData handGestureData, bool forced = false)
		{
			...
			if (handGestureData.m_currMode == MetaGesture.GRAB)
			{
				...
				handGestureData.m_controlledObj.SendMessage("OnRelease", handGestureData.m_handType, 1);
				handGestureData.m_controlledObj.SendMessage("OnGrabRelease", handGestureData.m_handType, 1);
			...
			else if (handGestureData.m_currMode == MetaGesture.PINCH)
			{
				...
				handGestureData.m_controlledObj.SendMessage("OnPinchRelease", handGestureData.m_handType, 1);
				if (this._enableGestureSounds && !forced)
				{
