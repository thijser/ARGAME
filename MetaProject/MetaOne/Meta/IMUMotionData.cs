	...
	internal class IMUMotionData
	{
		...
		public Vector3 Gravity
		{
			get
			{
				...
				result.set_Item(0, (float)num10);
				result.set_Item(1, -(float)(num11 * num9));
				result.set_Item(2, (float)(num9 * num12));
				result.Normalize();
				return result;
			}
		}
		...
		public void Reset()
		{
			...
			{
				this._correctionOrientation = Quaternion.get_identity();
			}
		...
		public bool LatchIMU(ref Quaternion oVirt2Gravity)
		{
			...
			if ((double)vector2.get_sqrMagnitude() < 1E-07)
			{
				return false;
			}
