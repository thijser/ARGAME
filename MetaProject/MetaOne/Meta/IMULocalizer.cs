...
namespace Meta
{
	public class IMULocalizer : Localizer
	{
		...
		public Vector3 imuOrientation
		{
			get
			{
				if (base.get_gameObject().get_activeSelf() && base.get_enabled())
				{
		...
		private void UpdateTargetGOTransform()
		{
			...
			this._targetGO.get_transform().set_rotation(rotation);
			Vector3 smoothedGravity = this._imuData.SmoothedGravity;
			smoothedGravity.Normalize();
			Debug.DrawLine(new Vector3(0f, 0f, 0f), 10f * smoothedGravity, Color.get_green());
		}
