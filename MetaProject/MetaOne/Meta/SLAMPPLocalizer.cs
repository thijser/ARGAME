...
namespace Meta
{
	internal class SLAMPPLocalizer : Localizer
	{
		...
		public void UpdateSLAMTimer()
		{
			...
			{
				this.slamTimerImage.set_enabled(true);
			...
			{
				this.slamTimerImage.set_enabled(false);
				if (this.slamArrowGO.get_activeSelf())
				{
		...
		public void UpdateTargetGOTransform()
		{
			...
			if (num == 0)
			{
				...
				this.slamArrowGO.get_transform().set_position(-this._targetGO.get_transform().get_position());
				this.StateTransition(SLAMPPLocalizer.SLAMLocalizerState.inTracking);
				this._targetGO.get_transform().set_rotation(this._slam2Gravity * this._quaternion);
				Vector3 smoothedGravity = this._imu.SmoothedGravity;
				smoothedGravity.Normalize();
				Debug.DrawLine(this._targetGO.get_transform().get_position(), 10f * smoothedGravity, Color.get_green());
			}
			else if (num == -1)
			{
