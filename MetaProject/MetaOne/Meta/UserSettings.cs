	...
	public class UserSettings : ScriptableObject
	{
		...
		public static void ResetUserSettings(ref UserSettings userProfile, UserSettings baseProfile = null, bool revertToDefault = false)
		{
			...
			if (baseProfile != null)
			{
				if (revertToDefault)
				{
					userProfile.Init(baseProfile.get_name().Replace(".userProfile", string.Empty));
				}
				else
				{
					baseProfile.DeepCopyTo(userProfile, false, false);
				}
				userProfile.set_name(baseProfile.get_name());
			}
		}
		...
		{
			userProfile = ScriptableObject.CreateInstance<UserSettings>();
			userProfile.set_name(user + ".userProfile");
		...
		public string UserProfileToJSON()
		{
			...
			fsData fsData;
			fsFailure fsFailure = fsSerializer.TrySerialize(typeof(UserSettings), this, ref fsData);
		...
		public bool JSONToUserProfile(string jsonString)
		{
			fsSerializer fsSerializer = new fsSerializer();
			fsData fsData;
			fsFailure fsFailure = fsJsonParser.Parse(jsonString, ref fsData);
			if (fsFailure.get_Failed())
			{
				throw new Exception(fsFailure.get_FailureReason());
			}
