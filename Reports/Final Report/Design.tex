\chapter{Design} \label{cha:design}
	...
	\section{Back-end design} \label{sec:backenddesign}
		...
		\subsection{The Core namespace} \label{ssec:corenamespace}
			...
			and code that helps the other two namespaces, the Core.Emitter
			namespace, which contains code for objects that emit laser beams,
			and the Code.Receiver namespace, which contains code for objects that
			receive laser beams and have to do something with these beams.
			There are several game objects that emit laser beams as well as receive
			them (as seen in \ref{sec:graphicaldesign}). This causes the
			Core.Emitter and Core.Receiver namespaces to have some coupling.
			However, the coupling is minor, and coupling is only between
			sub-namespaces in the same main namespace. As such, this should 
			not cause that much of a problem, when considered in a software
			engineering context.
	...
	\section{Game elements} \label{sec:graphicaldesign}
		...
		\subsection{Portal} \label{ssec:portal}
			...
			A portal is depicted in \ref{fig:portal}
			\begin{figure}[!ht]
				\centering
				\includegraphics[scale = 0.3]{Portal}
				\caption{An in-game picture of the portal. It looks like a 
				recolored mirror, but has different functionality. It has one
				black side and one green side. The black side is the side
				that allows laser beams to travel through it to the portal
				linked to it. The green side blocks laser beams completely.}
				\label{fig:portal}
			\end{figure}
