\chapter{Design} \label{cha:design}
	...
	\section{Main activities} \label{sec:mainactivities}
		...
		\subsection{A remote player wants to rotate a mirror} \label{ssec:remoterotatemirror}
			...
			(the color change will only be visible for the remote player responsible
			for clicking on that mirror, even other remote players will not be
			able to see this color change). A selected mirror can be rotated by
			using the "A" key (for rotation to the left, relative to the surface
			normal of the mirror) or the "D" key (for rotation to the right). The mouse can also be used for rotation by pressing the left and right mouse buttons on a mirror.
	...
	\section{Back-end design} \label{sec:backenddesign}
		...
		\subsection{The Core component} \label{ssec:corecomponent}
			...
			The \texttt{Core} component is further subdivided into three components:
...
\subsection{Experimental code: the RandomLevel component} \label{ssec:randomlevelcomponent}
			...
			discussing it with our coach, after which we agreed that the increase of the replay value of the game was not worth it. Another reason to abandon 
	...
	\section{Game elements} \label{sec:graphicaldesign}
		...
		\subsection{Laser target} \label{ssec:lasertarget}
			...
			next level once all targets are hit, provided that the level contains no checkpoints. An image of the target is shown in figure 
