...
TEST(AveragerTest, SingleValue) {
    ...
    EXPECT_EQ(averager.update(3.0f), 3.0f);
}

TEST(AveragerTest, TwoValues) {
    Averager<float> averager(2);
    EXPECT_EQ(averager.update(1.0f), 1.0f);
    EXPECT_EQ(averager.update(2.0f), 1.5f);
}
...
TEST(AveragerTest, HistoryLength) {
    ...
    EXPECT_EQ(averager.update(1.0f), 1.0f);
    EXPECT_EQ(averager.update(2.0f), 1.5f);
    EXPECT_EQ(averager.update(3.0f), 2.5f);
}
...
TEST(AveragerTest, Flush) {
    ...
    EXPECT_EQ(averager.update(1.0f), 1.0f);
    EXPECT_EQ(averager.update(2.0f), 1.5f);
    EXPECT_EQ(averager.update(20.0f), 20.0f);
}

TEST(AveragerTest, SinglePoint) {
    Averager<Point> averager(1);
    EXPECT_EQ(averager.update(Point(1, 1)), Point(1, 1));
}

TEST(AveragerTest, TwoPoints) {
    Averager<Point> averager(2);
    EXPECT_EQ(averager.update(Point(1, 1)), Point(1, 1));
    EXPECT_EQ(averager.update(Point(3, 9)), Point(2, 5));
}
...
TEST(AveragerTest, FlushPoints) {
    ...
    EXPECT_EQ(averager.update(Point(1, 1)), Point(1, 1));
    EXPECT_EQ(averager.update(Point(3, 9)), Point(2, 5));
    EXPECT_EQ(averager.update(Point(20, 9)), Point(20, 9));
}