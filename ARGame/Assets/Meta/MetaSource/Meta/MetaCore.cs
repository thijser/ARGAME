...
namespace Meta
{
	public class MetaCore : MonoBehaviour
	{
		...
		public bool enableCameraLight
		{
			...
			set
			{
				if (MetaSingleton<RenderingCameraManagerBase>.Instance.transform.FindChild("Light") != null)
				{
					MetaSingleton<RenderingCameraManagerBase>.Instance.transform.FindChild("Light").GetComponent<Light>().enabled = value;
				}
		...
		private static Transform GetTopParent(Transform obj)
		{
			...
			return MetaCore.GetTopParent(obj.parent);
		}
		...
		{
			if ((!(MetaSingleton<MetaLocalization>.Instance.currentLocalization == "IMULocalizer") && !(MetaSingleton<MetaLocalization>.Instance.currentLocalization == "SLAMLocalizer")) || MetaCore.Instance.transform.localRotation != Quaternion.identity)
			{
		...
		private void CheckForComponents()
		{
			...
			{
				this._HDMarkerTracker = (MarkerDetector)UnityEngine.Object.FindObjectOfType(typeof(MarkerDetector));
			}
			if (this._DeviceTextureSource == null)
			{
				this._DeviceTextureSource = (DeviceTextureSource)UnityEngine.Object.FindObjectOfType(typeof(DeviceTextureSource));
			}
			if (this._Hands == null)
			{
				this._Hands = (Hands)UnityEngine.Object.FindObjectOfType(typeof(Hands));
			}
			if (this._CanvasTracker == null)
			{
				this._CanvasTracker = (CanvasTracker)UnityEngine.Object.FindObjectOfType(typeof(CanvasTracker));
			}
		}
		...
		internal void Awake()
		{
			...
			if (num <= 0)
			{
				...
				UnityEngine.Debug.LogWarning("UnityEngine.Camera not properly connected. Mouselook backup controls enabled: Right-click-hold to look around, WASD to move.");
			}
			else
			{
			...
			{
				if (this._EventReceivers[i] != null && this._EventReceivers[i].enabled)
				{
