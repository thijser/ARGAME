...
namespace Meta
{
	internal class HandPointCloudDisplay : MonoBehaviour, IHandVisualiser<PointCloudData>
	{
		...
		private void SetParticlePoints(float[] points, int size, int offset)
		{
			...
			{
				this.m_cloud[i + offset].position = new Vector3(points[3 * i], points[3 * i + 1], points[3 * i + 2] - base.transform.localPosition.z);
				this.m_cloud[i + offset].color = this._particleColor;
				this.m_cloud[i + offset].size = this._particleSize;
			}
		}
	}
}
