...
namespace Meta
{
	...
	public class MetaLocalization : MetaSingleton<MetaLocalization>
	{
		...
		public string currentLocalization
		{
			get
			{
				return this._localizers[this._currentLocalization].name;
			}
			set
			{
				this.GetLocalizers();
				...
				{
					if (value == this._localizers[i].name)
					{
		...
		private void SwitchLocalizer()
		{
			for (int i = 0; i < this._localizers.Length; i++)
			{
				if (this._localizers[i] != null)
				{
					...
					{
						this._localizers[i].enabled = false;
					}
					else
					{
						this._localizers[i].enabled = true;
		...
		{
			this._localizers = base.transform.GetComponentsInChildren<Localizer>();
		}
