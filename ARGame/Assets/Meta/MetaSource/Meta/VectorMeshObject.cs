	...
	public class VectorMeshObject : MonoBehaviour
	{
		...
		private class Line
		{
			...
			public Line(Vector3 point1, Vector3 point2, Vector3 newNormal)
			{
				...
				this.direction = (this.pointA - this.pointB).normalized;
				...
				{
					num >>= 5;
					num ^= (uint)this.pointA[i].GetHashCode();
				}
				...
				{
					num >>= 5;
					num ^= (uint)this.pointB[j].GetHashCode();
				}
				this.LineHashCode |= (ulong)num;
			}
        ...
        public event TellParentDelegate TellParentReady;

        public event TellParentToChange TellParentToRemove;

        public event TellParentToChange TellParentToAdd;
		...
		private void InitVectrosityElements(List<Vector3> newlines, List<Vector3> newpoints)
		{
			...
			this.line.color = this.vectorStyle.lineColor;
			this.line.drawEnd = 0;
			...
			this.points = new VectorPoints("points " + base.gameObject.name, newpoints, this.vectorStyle.pointMaterial, this.vectorStyle.pointWidth);
			this.points.color = this.vectorStyle.pointColor;
			this.points.drawEnd = 0;
			this.points.drawTransform = base.transform;
		}

		private void ChangeDrawEndings(bool buildingIn = true)
		{
			float num = (float)this.line.points3.Count / this.fadeTime;
			float num2 = (float)this.points.points3.Count / this.fadeTime;
			float num3 = Time.realtimeSinceStartup - this.transitionStartTime;
			this.line.drawEnd = (!buildingIn) ? Mathf.RoundToInt((float)this.line.points3.Count - num3 * num) : Mathf.RoundToInt(num3 * num);
			this.points.drawEnd = (!buildingIn) ? Mathf.RoundToInt((float)this.points.points3.Count - num3 * num2) : Mathf.RoundToInt(num3 * num2);
		}

		public void BuildToggle()
		{
			if (this._state.State == VectorMeshObject.State.SHOWN || this._state.State == VectorMeshObject.State.HIDDEN)
			{
		...
		public void ShowOutline()
		{
			if (this.line != null)
			{
				...
				this.line.drawEnd = this.line.points3.Count;
				this.line.Draw3DAuto();
			}
			if (this.points != null)
			{
				this.points.active = true;
				this.points.drawEnd = this.points.points3.Count;
		...
		{
			base.GetComponent<Renderer>().enabled = state;
		}
		...
		private void AddChildBack(VectorMeshObject vmo)
		{
			...
			if (((this._state.State == VectorMeshObject.State.HIDDEN || this._state.State == VectorMeshObject.State.BUILDING_OUT) && vmo.CurrentState() != VectorMeshObject.State.HIDDEN) || ((this._state.State == VectorMeshObject.State.SHOWN || this._state.State == VectorMeshObject.State.BUILDING_IN) && vmo.CurrentState() != VectorMeshObject.State.SHOWN))
			{
				vmo.Flip();
			}
		}
		...
		public void InitChildren()
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				this.AddChild(meshRenderer.gameObject);
			}
		}
		...
		private void AddChild(GameObject go)
		{
			...
			{
				go.gameObject.AddComponent<VectorMeshObject>();
			}
		...
		public void ApplyVectorStyle()
		{
			if (!this.nonRenderingController)
			{
				...
				{
					Material material = base.GetComponent<Renderer>().materials[0];
					Color color = default(Color);
					if (this.preserveColors && material.HasProperty("_Color"))
					{
						color = material.color;
					}
					base.GetComponent<Renderer>().materials[i] = this.vectorStyle.surfaceMaterial;
					if (this.preserveColors && material.HasProperty("_Color"))
					{
						base.GetComponent<Renderer>().materials[i].color = color;
					}
				}
				base.GetComponent<Renderer>().material = this.vectorStyle.surfaceMaterial;
			}
			this.vectorStyle.lineMaterial.color = this.vectorStyle.lineColor;
			this.vectorStyle.pointMaterial.color = this.vectorStyle.pointColor;
		}
		...
		private void MakeFrameAndPoints(Mesh mesh, out List<Vector3> lines, out List<Vector3> points)
		{
			...
			{
                UnityEngine.Debug.Log("Ignoring Mesh as it is too big");
				return;
			}
		...
		private void InitVectorMeshObject()
		{
			...
			if (!this.nonRenderingController)
			{
				...
				this.MakeFrameAndPoints(base.GetComponent<MeshFilter>().mesh, out newlines, out newpoints);
