...
namespace Meta
{
	internal class KalmanFilter
	{
		...
		{
			Quaternion rotation = kalmanTransform.rotation;
			Vector3 position = kalmanTransform.position;
			...
			{
				KalmanFilter.positionDelta = Vector3.Distance(KalmanFilter.previousTransform[TransformID].localPosition, kalmanTransform.localPosition);
				KalmanFilter.m_KalmanVelocity = KalmanFilter.positionDelta * KalmanFilter.positionDeltaToKalmanMultiplier * Time.deltaTime;
				KalmanFilter.rotationDeltaDegrees = Quaternion.Angle(KalmanFilter.previousTransform[TransformID].localRotation, kalmanTransform.localRotation);
			}
			if (KalmanFilter.positionDelta >= 0.01f)
			{
				KalmanFilter.previousTransform[TransformID].localPosition = kalmanTransform.localPosition;
			}
			else
			{
			...
			{
				KalmanFilter.previousTransform[TransformID].localRotation = kalmanTransform.localRotation;
			}
			...
			{
				kalmanTransform.rotation = new Quaternion(KalmanFilter.xrot, KalmanFilter.yrot, KalmanFilter.zrot, KalmanFilter.wrot);
			}
			else
			{
				Debug.Log("ERROR: UpdateTransform: transform.rotation = new Quaternion(xrot, yrot, zrot, wrot); // xrot can't be NaN");
			}
			kalmanTransform.position = new Vector3(KalmanFilter.x, KalmanFilter.y, KalmanFilter.z);
		}
	}
}
